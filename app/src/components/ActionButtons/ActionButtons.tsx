...
const ActionButtons: React.FC<ActionButtonsProps> = ({buttons, isSubscriptionExpired, isMoreThan3days}) => {
    ...
    return (
        <div className={styles.actionButtons}>
            {buttons.map((button) => {
                ...
                return <div key={button.id} className={styles.actionBtn} onClick={() => navigate(button.url)} style={{
                    '--color': button.borderColor || getButtonColor(button),
                    '--background-color': getButtonColor(button, true, true),
                } as React.CSSProperties}>
                    {Logo ? <Logo height="29px" width="116px" color={getButtonColor(button)}
                                  style={{
                                      alignSelf: "flex-start",
                                      "--content-color": getContentColor(button),
                                  } as React.CSSProperties}/> :
                        <img height="29px" width="116px" style={{alignSelf: "flex-start"}}
                             src={"/assets/crd-" + button.id + ".svg"} alt=" "/>
                    }
                    <WideButton color={getButtonColor(button, false)} text={button.buttonLabel}
                                onClick={() => navigate(button.url)}
                                buttonStyle={{
                                    color: getContentColor(button), ...buttonStyles
                                }}
                    />
                </div>
            })}
