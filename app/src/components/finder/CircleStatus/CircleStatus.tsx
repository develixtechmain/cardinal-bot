...
export default function CircleStatus({activeCount}: Props) {
    ...
    const angles = Array.from({length: TOTAL_CIRCLES}, (_, i) =>
        (180 - (i * 180) / (TOTAL_CIRCLES - 1)) * (Math.PI / 180)
    );
    ...
    const points = angles.map((a) => ({
        x: leadsCircleCenterX + CIRCLES_RADIUS * Math.cos(a),
        y: leadsCircleCenterY - CIRCLES_RADIUS * Math.sin(a),
    }));
    ...
    return (
        <div ref={ref} className={styles.container}>
            {points.map((p, i) => (
                <div
                    key={i}
                    className={`${styles.smallCircle} ${i < activeCount ? styles.active : styles.disabled}`}
                    style={{
                        left: p.x,
                        top: p.y,
                    }}
            ...
            >
                <LeadsBackground
                    ...
                    style={{
                        ...
                        top: 19,
                        left: -24,
                        "--content-color": isExpired ? "#232323" : "#44128A",
                    } as React.CSSProperties}
                />
                <div
                    className={`${styles.coreCount}`}
                    style={{
                        "--count-color": "#F2F2F2",
                    } as React.CSSProperties}
                >
