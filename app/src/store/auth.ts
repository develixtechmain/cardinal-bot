...
type AuthStore = {accessToken: string | undefined; refreshToken: string | undefined; clearTokens: () => void; refreshAccessToken: () => Promise<string>};
...
const auth = async (set: {
    ...
    if (tg) {
        try {
            ...
            set({accessToken: auth.access_token, refreshToken: auth.refresh_token});
