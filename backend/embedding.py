...
async def save_tags(user_id, task_id, tags):
    try:
        res = await client.post("/vectors/tags", json={
            "tags": tags,
            "taskId": str(task_id),
            "userId": str(user_id)
        })
...
async def accept_recommendation(recommendation_id):
    try:
        res = await client.patch(f"/api/learning/{recommendation_id}/success")
        logger.debug(f"Response status: {res.status_code}, Response: {res.text}")
        if res.status_code != 200:
...
async def decline_recommendation(recommendation_id):
    try:
        res = await client.patch(f"/api/learning/{recommendation_id}/fail")
        logger.debug(f"Response status: {res.status_code}, Response: {res.text}")
        if res.status_code != 200:
